...
func (ms *MergerSuite) TestMerger_New() {
	...
	for _, tc := range testcases {
		ms.T().Run(tc.name, func(t *testing.T) {
			m, err := sortmerger.NewMerger(sortmerger.NewSortColumn("id", utils.ASC))
...
func (ms *MergerSuite) TestMerger_Merge() {
	testcases := []struct {
	...
	}{
		{
			...
			getMerger: func() (merger.Merger, error) {
				return sortmerger.NewMerger(sortmerger.NewSortColumn("id", utils.ASC))
			},
			GetRowsList: func() []*sql.Rows {
				return []*sql.Rows{}
			},
...
func (ms *MergerSuite) TestRows_ScanAndErr() {
	ms.T().Run("未调用Next，直接Scan，返回错", func(t *testing.T) {
		...
		require.NoError(t, err)
		rowsList := []*sql.Rows{r}
		merger, err := sortmerger.NewMerger(sortmerger.NewSortColumn("id", utils.ASC))
		require.NoError(t, err)
