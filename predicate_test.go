...
func TestPredicate_C(t *testing.T) {
	...
	testCases := []CommonTestCase{
		{
			name:    "empty",
			builder: NewSelector(db).Select(Columns("Id")).From(&TestModel{Id: 10}).Where(),
			wantSql: "SELECT `id` FROM `test_model`;",
		},
		{
			name: "multiples",
			// 在传入多个 Predicate 的时候，我们认为它们是用 and 连接起来的
			builder: NewSelector(db).Select(Columns("Id")).From(&TestModel{Id: 10}).
		...
		{
			name:     "Avg with EQ",
			builder:  NewSelector(db).Select().From(&TestModel{}).GroupBy("FirstName").Having(Avg("Age").EQ(18)),
			wantSql:  "SELECT `id`,`first_name`,`age`,`last_name` FROM `test_model` GROUP BY `first_name` HAVING AVG(`age`)=?;",
			wantArgs: []interface{}{18},
		},
		{
			name:     "Max with NEQ",
			builder:  NewSelector(db).Select().From(&TestModel{}).GroupBy("FirstName").Having(Max("Age").NEQ(18)),
			wantSql:  "SELECT `id`,`first_name`,`age`,`last_name` FROM `test_model` GROUP BY `first_name` HAVING MAX(`age`)!=?;",
			wantArgs: []interface{}{18},
		},
		{
			name:     "Min with LT",
			builder:  NewSelector(db).Select().From(&TestModel{}).GroupBy("FirstName").Having(Min("Age").LT(18)),
			wantSql:  "SELECT `id`,`first_name`,`age`,`last_name` FROM `test_model` GROUP BY `first_name` HAVING MIN(`age`)<?;",
			wantArgs: []interface{}{18},
		},
		{
			name:     "Sum with LTEQ",
			builder:  NewSelector(db).Select().From(&TestModel{}).GroupBy("FirstName").Having(Sum("Age").LTEQ(18)),
			wantSql:  "SELECT `id`,`first_name`,`age`,`last_name` FROM `test_model` GROUP BY `first_name` HAVING SUM(`age`)<=?;",
			wantArgs: []interface{}{18},
		},
		{
			name:     "Count with GT",
			builder:  NewSelector(db).Select().From(&TestModel{}).GroupBy("FirstName").Having(Count("Age").GT(18)),
			wantSql:  "SELECT `id`,`first_name`,`age`,`last_name` FROM `test_model` GROUP BY `first_name` HAVING COUNT(`age`)>?;",
			wantArgs: []interface{}{18},
		},
		{
			name:     "Avg with GTEQ",
			builder:  NewSelector(db).Select().From(&TestModel{}).GroupBy("FirstName").Having(Avg("Age").GTEQ(18)),
			wantSql:  "SELECT `id`,`first_name`,`age`,`last_name` FROM `test_model` GROUP BY `first_name` HAVING AVG(`age`)>=?;",
			wantArgs: []interface{}{18},
		},
		{
			name:     "multiples aggregate functions",
			builder:  NewSelector(db).Select().From(&TestModel{}).GroupBy("FirstName").Having(Avg("Age").GTEQ(18).And(Sum("Age").LTEQ(30))),
...
func ExampleNot() {
	db := NewDB()
	query, _ := NewSelector(db).Select(Columns("Id")).From(&TestModel{Id: 10}).Where(Not(C("Id").EQ(18))).Build()
...
func ExamplePredicate_And() {
	db := NewDB()
	query, _ := NewSelector(db).Select(Columns("Id")).From(&TestModel{
		Id: 10,
