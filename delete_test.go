...
func TestDeleter_Build(t *testing.T) {
	testCases := []CommonTestCase{
		{
			name:    "no where",
			builder: NewDB().Delete().From(&TestModel{}),
			wantSql: "DELETE FROM `test_model`;",
		},
		{
			name:     "where",
			builder:  NewDB().Delete().From(&TestModel{}).Where(C("Id").EQ(16)),
...
func ExampleDeleter_Build() {
	query, _ := NewDB().Delete().From(&TestModel{}).Build()
...
func ExampleDeleter_Where() {
	query, _ := NewDB().Delete().From(&TestModel{}).Where(C("Id").EQ(12)).Build()
